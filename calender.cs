...
class Program //contains the main()method, the entry of every C# application
{
    ...
    static void Main() //static = run it directly, void = no need to return anything, Main() = Action!
    {
        while (true) //keeps running until the user chooses to exit 
        {
            ...
            Console.Write ("\nSelect an option: ");// Write = print and stay on the same line 
            ...
            switch (input) //switch = check which option the user selected and do the corresponding action
            {
                ...
                default: 
                    ...
                    break; //default = do this if the user types something that is not one of the options provided 
            }
        }
    }
    ...
    foreach (var booking in bookings) //foreach = loop through each booking in the bookings list
    {
        Console.WriteLine($"\n{booking.Name} - {booking.Topic.Title} - {booking.Time}"); //print out the booking details 
    }
    }
}
